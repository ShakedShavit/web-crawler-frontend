...
function HomePage() {
    ...
    const getTreeInterval = async () => {
        while (isCrawling && !!queueName && !isDeletingQueue) {
            try {
                ...
                if (!!crawlInfo) {
                    dispatchTreeState(
                        updateTreeAction(crawlInfo?.tree?.length > 0 ? crawlInfo.tree : "{}")
                    );
                }
    ...
    return (
        <div>
            ...
            <div className="show-form-button" onClick={changeFormShow}>
                {showForm ? "-" : "+"}
            </div>
            {showForm && (
                <CrawlerForm
            ...
            {treeState !== "{}" && (
                <div className="tree">
                    <ReactJsonPrint dataString={treeState} expanded={true} />
                </div>
            )}
        </div>
    );
